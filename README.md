...
library(simr)
model1 = lmer(y1 ~ time*intervention + (time|subject), data = d)
model2 = lmer(y1 ~ time + (1|subject), data = d)
model3 = lmer(y1 ~ time + (time|subject), data = d)
model4 = lmer(y1 ~ time + intervention + (time | subject), data = d)
model5 = lmer(y1 ~ intervention + time + (time | subject), data = d)
model6 = lmer(y1 ~ time + intervention + time*intervention + (time | subject), data = d)
summary(model5)


fixef(model4)["time"] = .26
set.seed(123)

powerCurve(model2, along = "subject", nsim = 10)
powerCurve(model3, along = "subject", nsim = 10)
powerCurve(model4, along = "subject", nsim = 10)
powerCurve(model5, along = "subject", nsim = 10)
powerCurve(model1, along = "time", nsim = 10)
powerCurve(model6, along = "subject", nsim = 10, test = fixed("intervention"))
powerCurve(model6, along = "subject", nsim = 10, test = fixed("time:intervention"))
# When you run the below, it doesn't work, which makes me think that the code below is accessing the the correct parameter estimate:
powerCurve(model6, along = "subject", nsim = 10, test = fixed("timeintervention"))



powerSim(model1,fixef(model1)["time:intervention"], nsim = 10)
```
